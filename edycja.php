...
if ($_SERVER["REQUEST_METHOD"] == "POST" and isset($_GET["wyd"])) {
	...
	$sql = mysqli_query($conn, "UPDATE wyd SET wyd='$xwyd', tel='$xtel', mail='$xmail', adres='$xadres' WHERE idwyd='$wyd'") or die(mysqli_error($conn));
...
?>
	<form method="post" action="index.php?plik=edycja&wyd=<?php echo $wyd; ?>"><br>
		...
		Numer telefonu
		<Br><input type="tel" required name="xtel" value="<?php echo $wiersz["tel"]; ?>" placeholder="123456789"><Br>
		E-mail
		<Br><input type="email" required name="xmail" value="<?php echo $wiersz["mail"]; ?>"><Br>
		Adres
		<Br><input type="text" required name="xadres" value="<?php echo $wiersz["adres"]; ?>"><Br>
		<P><input type="submit" value="Edytuj">
	</form>
